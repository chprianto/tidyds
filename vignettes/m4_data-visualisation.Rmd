...
ncol(un_smp)
range(un_smp$tahun)
range(un_smp$bahasa_indonesia)
range(un_smp$bahasa_inggris)
range(un_smp$ipa)
```
...
```{r}
ggplot(un_smp, aes(x = bahasa_indonesia, y = matematika)) +
  geom_point(aes(colour = status, size = jumlah_peserta), alpha = 0.3)
```
...
mtk_vs_ind <- ggplot(un_smp, aes(x = bahasa_indonesia, y = matematika)) +
  ...
  labs(
    ...
    size = "# peserta",
    title = "Nilai Ujian Bahasa Indonesia dan Matematika",
